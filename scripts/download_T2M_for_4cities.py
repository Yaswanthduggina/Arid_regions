...
parameter = "T2M"   # Mean air temperature

for city, coord in stations.items():
    print(f"\n🌡 Downloading T2M for {city}")
    ...
    for year in range(2001, 2026):
        ...
        if r.status_code != 200:
            print(r.text)
            raise RuntimeError(f"Failed for {city} in {year}")

        yearly = r.json()["properties"]["parameter"][parameter]
        city_data.update(yearly)

    # Save to city folder
    out_path = f"data/raw/{city}_data/{city}_T2M_hourly.json"
    with open(out_path, "w") as f:
        json.dump(city_data, f)

    print(f"✔ Saved {out_path}")

print("\n✅ T2M DOWNLOAD COMPLETE")
